...
# define function
my_mgseaNb <- function(object = NULL,
                   subPlot = 3,
                   lineSize = 1,
                   geneSetID = NULL,
                   rmSegment = FALSE,
  ...
  if (newGsea == FALSE) {
    ...
    if (subPlot == 1) {
      ...
      # combine
      if (rmHt == FALSE){
        pres=mg_merge_plot(pLabelOut, pseg_ht, ncol = 1, heights = c(0.8, 0.2),align = 'v')
        ...
        pres=mg_merge_plot(pLabelOut, pseg, ncol = 1, heights = c(0.8, 0.2),align = 'v')
        # pres <- aplot::plot_list(
      ...
      # combine
      if (rmHt == FALSE){
        pres=mg_merge_plot(pLabelOut, pseg_ht1, prank, ncol = 1, heights = c(0.5, 0.2, 0.3),align = 'v')
        # pres <-
        #   aplot::plot_list(
        ...
        pres=mg_merge_plot(pLabelOut, pseg, prank, ncol = 1, heights = c(0.5, 0.2, 0.3),align = 'v')
        # pres <-
        #   aplot::plot_list(
    ...
    # combine
    if (rmHt == FALSE) {
      pres=mg_merge_plot(pLabelOut, ht, ncol = 1, heights = c(0.9, 0.1),align = 'v')
      # pres <- aplot::plot_list(
  ...
  # whether add gene expression heatmap
  if(add.geneExpHt == TRUE){
    pfinal=mg_merge_plot(pres +xlab(''), fght, ncol = 1,heights = c(1-ght.relHight, ght.relHight),align = 'v')
